...
function displayResult(res, options) {
  ...
  const binariesIssues = binariesIssuesForDisplay(res);
  ...
  const groupedVulnInfoOutput = sortedGroupedVulns.map((vuln) => {
...
function createFixedInText(groupedVuln) {
  const vulnerableRange = groupedVuln.list[0].semver.vulnerable[0];
